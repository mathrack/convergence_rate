...
C compile ifort wa.f -r8  -I./fftw-3.3.4/include -L./fftw-3.3.4/lib -lfftw3
...
C PROCESS MEAN SPANWISE VELOCITY FIRST
       do i=1,200000
         read(2115,'(512E24.16)') (u(K),K=1,KM)
C make symmetric
         do j=1,km/2+1
...
C PROCESS SPANWISE HEAT FLUX
         read(2124,'(512E24.16)') (v(K),K=1,KM)
c make asymmetric
         do j=1,km/2+1
...
C PROCESS MEAN SPANWISE VELOCITY FIRST
       do i=200001,400000
         read(2215,'(512E24.16)') (u(K),K=1,KM)
C make symmetric
         do j=1,km/2+1
          tmp=(u(km+1-j)+u(j))/DBLE(2)
          u(j)=tmp
...
C PROCESS SPANWISE HEAT FLUX
         read(2224,'(512E24.16)') (v(K),K=1,KM)
c make asymmetric
         do j=1,km/2+1
          tmp=(v(km+1-j)-v(j))/DBLE(2)
          v(j)=-tmp
