...
This is the expected convergence rate for an uncorrelated signal, and for a weakly correlated one approaching independence at large separations, such as a turbulent signal (Theorem 27.4 in Billingsley \citep{billingsley2008probability}).
...
\end{eqnarray}
The residual error $E_U$, also defined by Andrade et al. \citep{andrade}, can be obtained by integrating $E_R$.
...
\begin{table}
\caption{\label{tab_conv_rate}Convergence rate for individual and global quantities, averaging time $T^+>2700$.}
\begin{ruledtabular}
\begin{tabular}{rcccc}
 & ${\parallel{E_0^X}\parallel}^2$ & ${\parallel{E_0^Z}\parallel}^2$ & $Z-Conv$ & $Z-diff$ \\
\hline
Convergence Rate & $T^{-2.0002}$ & $T^{-2.0016}$ & $T^{-1.3302}$ & $T^{-1.1340}$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
As a side note, the authors would like to stress that convergence rates correspond to asymptotic limits and long averaging time can be necessary to observe them, especially for low probability events (Bauer et al. \citep{bauer2017convergence}).
